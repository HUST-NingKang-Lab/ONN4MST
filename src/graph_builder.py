...
class model(object):
  ...
  def build_graph(self):
    ...
    with self.g.as_default():
      ...
      base0_1 = tf.layers.dense(self.x, 1024, activation=tf.nn.relu)
      base0 = tf.layers.dense(base0_1, 512, activation=tf.nn.relu)
      ...
      self.y_pred = tf.concat([self.pred0, self.pred1, self.pred2, self.pred3, self.pred4], axis=1)
      self.logits = tf.concat([self.otlg0, self.otlg1, self.otlg2, self.otlg3, self.otlg4], axis=1)
      self.y_true = tf.concat([self.y_0, self.y_1, self.y_2, self.y_3, self.y_4], axis=1)
  ...
  def init_session(self):
    self.sess = tf.compat.v1.Session(config=self.config,graph=self.g)
