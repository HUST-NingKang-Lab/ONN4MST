...
if args.mode == 'check':
	...
	# fix issue 1
	biomes_fixed = map(lambda x: x.replace('Host-associated', 'Host_associated').\
					   replace('Oil-contaminated', 'Oil_contaminated').\
					   replace('Non-marine', 'Non_marine'), os.listdir(args.input_dir))
	...
	biomes_fixed = map(lambda x: x.replace('Host-associated', 'Host_associated').\
					   replace('Oil-contaminated', 'Oil_contaminated').\
					   replace('Non-marine', 'Non_marine'), loader.paths_keep)
